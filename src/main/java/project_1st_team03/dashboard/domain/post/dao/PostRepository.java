...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "select p from Post p join fetch p.member where p.status = 'NORMAL'"
    ...
    @Query(value = "select p from Post p join fetch p.member m where m.email = :email and p.status = 'NORMAL'")
